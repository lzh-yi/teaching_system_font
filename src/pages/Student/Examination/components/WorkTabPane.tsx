            ...
            <Col
              ...
              onClick={() => {
                if (item?.workstatistics?.submitStatus === '1')
                  history.push(
                    `/student/examination/detail?work_status=${item?.workstatistics?.submitStatus}&work_group=${item?.workGroup?.id}&work_statistics=${item?.workstatistics?.id}`,
                  );
                if (item?.workstatistics?.submitStatus === '0')
                  history.push(
                    `/student/examination/doing?work_status=${item?.workstatistics?.submitStatus}&work_group=${item?.workGroup?.id}&work_statistics=${item?.workstatistics?.id}`,
                  );
              }}
            >
